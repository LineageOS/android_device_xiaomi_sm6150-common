...
template <typename T>
int registerService(const std::string& instanceName) {
    std::shared_ptr<T> service = ndk::SharedRefBase::make<T>();
    return AServiceManager_addService(service->asBinder().get(), instanceName.c_str());
}
...
int main() {
    ...
    std::shared_ptr<PictureAdjustment> pictureAdjustment = ndk::SharedRefBase::make<PictureAdjustment>(controller);
    std::string instance = std::string() + PictureAdjustment::descriptor + "/default";
    if (AServiceManager_addService(pictureAdjustment->asBinder().get(), instance.c_str()) != STATUS_OK) {
    ...
    std::shared_ptr<SunlightEnhancement> sunlightEnhancement = ndk::SharedRefBase::make<SunlightEnhancement>();
    instance = std::string() + SunlightEnhancement::descriptor + "/default";
    if (AServiceManager_addService(sunlightEnhancement->asBinder().get(), instance.c_str()) != STATUS_OK) {
